  ...
  return {
    ...
    init: function() {
      ...
      function saveRow(oTable, nRow, id) {
        ...
        type = id ? 'PATCH' : "POST";
        url = id ? ('/sales/' + id) : '/sales';
        var data = {};
        console.log(jqInputs)
        if (jqInputs[0].value) data['sale[name]'] = jqInputs[0].value;
        if (jqInputs[1].value) data['sale[tel]'] = jqInputs[1].value;
        if (jqInputs[2].value) data['sale[gender]'] = jqInputs[2].value;
        if (jqInputs[3].value) data['sale[age]'] = jqInputs[3].value;
        if (jqInputs[4].value) data['sale[join_time]'] = jqInputs[4].value;
        $.ajax({
          url: url,
          data: data,
          type: type,
          ...
          success: function(res) {
            oTable.fnUpdate('<a href="/sales/'+ res.id +'" >' + res.name + '</a>', nRow, 0, false);
