  ...
  return {
    ...
    init: function() {
      ...
      function saveRow(oTable, nRow, id) {
        ...
        type = id ? 'PATCH' : "POST";
        url = id ? ('/car_heads/' + id) : '/car_heads';
        var data = {};
        console.log(jqInputs)
        if (jqInputs[0].value) data['car_head[car_number]'] = jqInputs[0].value;
        if (jqInputs[1].value) data['car_head[head_type]'] = jqInputs[1].value;
        if (jqInputs[2].value) data['car_head[brand]'] = jqInputs[2].value;
        if (jqInputs[3].value) data['car_head[fuel_type]'] = jqInputs[3].value;
        if (jqInputs[4].value) data['car_head[status]'] = jqInputs[4].value;
        if (jqInputs[5].value) data['car_head[loc]'] = jqInputs[5].value;
        $.ajax({
          url: url,
          data: data,
          type: type,
          ...
          success: function(res) {
            oTable.fnUpdate(res.car_number, nRow, 0, false);
            oTable.fnUpdate(res.head_type, nRow, 1, false);
            oTable.fnUpdate(res.brand, nRow, 2, false);
            oTable.fnUpdate(res.fuel_type, nRow, 3, false);
